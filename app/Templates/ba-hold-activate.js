...
/**
 * Template: Berita Acara MDS HOLD Diaktifkan Kembali
 *
 * Field payload.data yang diharapkan:
 * - letterNo          (string, wajib)   : Nomor surat, contoh "098/OMI-TM/BAK/IV/2026"
 * - region            (string, wajib)   : Wilayah, contoh "JTU"
 * - reactivateDate    (string/ISO)      : Tanggal aktif kembali, contoh "2026-04-06"
 ...
 * - holdReason        (string)          : Alasan HOLD sebelumnya
 ...
 */
module.exports = function baHoldActivateTemplate(payloadData = {}) {
  const {
    letterNo = '',
    region = '',
    reactivateDate = '',
  ...
  return {
    ...
    content: [
      ...
      {
        text: [
          'Pemberitahuan MDS HOLD DIAKTIFKAN kembali untuk Wilayah ',
          { text: upper(region) || '-', bold: true },
          ' per tanggal ',
          { text: formatDateUpper(reactivateDate), bold: true },
...

      {
        table: {
          ...
          body: [
            ...
            detailRow('Alasan HOLD', holdReason || '-'),
          ],
        },
        layout: {
          hLineWidth: () => 0,
