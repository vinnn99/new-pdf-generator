...
/**
 * Template: Berita Acara Resign MDS
 *
 * Field payload.data yang diharapkan:
 * - letterNo           (string, wajib)   : Nomor surat
 * - region             (string, wajib)   : Wilayah
 * - mdsName            (string, wajib)   : Nama MDS
 * - mdsCode            (string, wajib)   : Kode MDS
 * - nik                (string)          : NIK MDS
 * - birthDate          (string/ISO)      : Tanggal lahir
 * - effectiveResignDate(string/ISO)      : Tanggal efektif resign
 * - status             (string, wajib)   : Status MDS
 * - mdsCategory        (string)          : Kategori MDS
 * - outletFrom         (string)          : Outlet asal
 * - resignReason       (string)          : Alasan resign
 ...
 */
module.exports = function baResignTemplate(payloadData = {}) {
  const normalizedRegion = payloadData.region || payloadData.area || payloadData.wilayah || ''
  const normalizedBirthDate = payloadData.birthDate || payloadData.dateOfBirth || payloadData.tanggalLahir || ''
  const normalizedEffectiveResignDate = payloadData.effectiveResignDate || payloadData.resignDate || payloadData.resignEffectiveDate || ''
  const normalizedMdsCategory = payloadData.mdsCategory || payloadData.category || payloadData.kategoriMds || ''
  const normalizedOutletFrom = payloadData.outletFrom || payloadData.outlet || payloadData.dariOutlet || ''
  const normalizedResignReason = payloadData.resignReason || payloadData.reason || payloadData.alasanResign || ''
  ...
  const {
    ...
    effectiveResignDate = normalizedEffectiveResignDate,
    status = '',
    mdsCategory = normalizedMdsCategory,
    outletFrom = normalizedOutletFrom,
    resignReason = normalizedResignReason,
    location = 'Jakarta',
  ...
  const formatDateTitle = (val) => {
    ...
    return `${String(d.getDate()).padStart(2, '0')} ${monthsTitle[d.getMonth()]} ${d.getFullYear()}`
  }
  ...
  return {
    ...
    content: [
      ...
      {
        text: [
          'Pemberitahuan MDS Resign untuk Outlet Wilayah ',
      ...
      },
      {
        table: {
          ...
          body: [
            ...
            detailRow('Kode MDS', upper(mdsCode)),
            detailRow('NIK', upper(nik)),
            detailRow('Tanggal Lahir', formatDateUpper(birthDate)),
            detailRow('Tanggal Efektif Resign', formatDateUpper(effectiveResignDate)),
            detailRow('Status', upper(status)),
            detailRow('Kategori MDS', upper(mdsCategory)),
            detailRow('Dari Outlet', upper(outletFrom)),
            detailRow('Alasan MDS Resign', upper(resignReason))
          ]
        },
        layout: {
          hLineWidth: () => 0,
