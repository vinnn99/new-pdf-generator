...
/**
 * Template: Berita Acara Request ID MDS
 *
 * Field payload.data yang diharapkan:
 * - letterNo         (string, wajib)   : Nomor surat, contoh "102/OMI-TM/BAK/IV/2026"
 * - area             (string)          : Area/wilayah MDS, contoh "JTU"
 * - mdsName          (string, wajib)   : Nama MDS
 * - nik              (string, wajib)   : NIK MDS
 * - birthDate        (string/ISO)      : Tanggal lahir
 * - joinDate         (string/ISO)      : Tanggal masuk
 * - status           (string)          : Status MDS, contoh "MOBILE"
 * - stores           (string/array)    : Daftar toko/outlet, bisa array atau string dipisah koma
 * - reason           (string)          : Alasan pengajuan Request ID
 ...
 */
module.exports = function baRequestIdTemplate(payloadData = {}) {
  const {
    letterNo = '',
    area = '',
    mdsName = '',
  ...
  return {
    ...
    content: [
      ...
      area
        ? { text: `Request ID MDS Area ${upper(area)}:`, style: 'intro', alignment: 'left', margin: [0, 0, 0, 12] }
        : {},

      {
        table: {
          ...
          body: [
            ...
            detailRow('Tanggal Masuk', formatDateUpper(joinDate)),
            detailRow('Status', upper(status)),
            detailRow('Toko', normalizeStores(stores) || '-'),
