...
/**
 * Template: Berita Acara Batal Join MDS
 *
 * Field payload.data yang diharapkan:
 * - letterNo         (string, wajib)   : Nomor surat, contoh "001/POMI/BABJ/IV/2026"
 * - region           (string, wajib)   : Wilayah, contoh "SMS"
 * - cancelJoinDate   (string/ISO)      : Tanggal batal join
 ...
 * - reason           (string)          : Alasan batal join
 ...
 */
module.exports = function baCancelJoinTemplate(payloadData = {}) {
  const {
    letterNo = '',
    region = '',
    cancelJoinDate = '',
  ...
  return {
    ...
    content: [
      ...
      {
        text: [
          'Pemberitahuan Batal Join MDS untuk Wilayah ',
          { text: upper(region) || '-', bold: true },
          ' per tanggal ',
          { text: formatDateUpper(cancelJoinDate), bold: true },
...

      {
        table: {
          ...
          body: [
            ...
            detailRow('Alasan Batal Join', upper(reason) || '-'),
          ],
        },
        layout: {
          hLineWidth: () => 0,
