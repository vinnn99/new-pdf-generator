...
/**
 * Template: Berita Acara Rolling MDS
 *
 * Field payload.data yang diharapkan:
 * - letterNo         (string, wajib)   : Nomor surat, contoh "099/OMI-TM/BAK/IV/2026"
 * - region           (string, wajib)   : Wilayah, contoh "JTU"
 * - rollingDate      (string/ISO)      : Tanggal rolling, contoh "2026-04-07"
 * - mdsName          (string, wajib)   : Nama MDS
 * - mdsCode          (string, wajib)   : Kode MDS
 * - status           (string)          : Status MDS, contoh "STAY"
 * - outletFrom       (string)          : Outlet sebelumnya
 * - outletTo         (string)          : Outlet penempatan baru
 * - reason           (string)          : Alasan rolling
 ...
 */
module.exports = function baRollingTemplate(payloadData = {}) {
  const {
    letterNo = '',
    region = '',
    rollingDate = '',
  ...
  return {
    ...
    content: [
      ...
      {
        text: [
          'Pemberitahuan Rolling MDS untuk Wilayah ',
          { text: upper(region) || '-', bold: true },
          ' per tanggal ',
          { text: formatDateUpper(rollingDate), bold: true },
...

      {
        table: {
          ...
          body: [
            ...
            detailRow('Outlet Sebelumnya', upper(outletFrom)),
            detailRow('Outlet Penempatan', upper(outletTo)),
